...
function addtoorderpage(id) {
  // localStorage'tan 'orderpage' veya 'myorders' alınır veya boş bir dizi oluşturulur
  let orderpage = JSON.parse(localStorage.getItem('orderpage')) || [];
  let myorders = JSON.parse(localStorage.getItem('myorders')) || [];

  let productToAdd = products.find(item => item.id == id);

  orderpage.push(productToAdd);

  const maxItemsInOrderPage = 1;
  if (orderpage.length > maxItemsInOrderPage) {
    orderpage.shift();
  }

  myorders.push(productToAdd);

  localStorage.setItem('orderpage', JSON.stringify(orderpage));
  localStorage.setItem('myorders', JSON.stringify(myorders));

  // orderpage.html sayfasına yönlendirilir
